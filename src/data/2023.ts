...
const problema_1: ProblemaType = {
    titlu: "Numărul N",
    enunt_markdown: `
Considerăm numărul N = 1! + 2! + ... + 2023!. Care este ultima cifră a numărului N?
`,
    variante: ["0", "1", "3", "5", "8"],
    raspuns: "3",
    dificultate: "usor"
...
const problema_2: ProblemaType = {
    ...
    enunt_markdown: `
Un număr este puternic dacă este strict mai mare decât răsturnatul său. De exemplu, 10 și 21 sunt numere puternice, dar 1 și 34 nu sunt. Răsturnatul lui 10 este 1. Câte numere pozitive până în 2023 (inclusiv) sunt puternice?
`,
...
const problema_3: ProblemaType = {
    titlu: "RedDrink",
    enunt_markdown: `
Matei se pregătește să învețe asiduu pentru examenul de Bacalaureat. Pentru a avea mai multă energie, el și-a cumpărat 100 de doze de băutură energizantă RedDrink. Matei preferă această băutură și pentru promoția de care poate beneficia, respectiv dacă reciclează 7 doze de aluminiu goale de RedDrink primește una plină la schimb. El și-a făcut un plan să bea 3 doze de RedDrink pe zi (dimineața, la prânz și seara) pentru a susține un efort mare de învățare. Dacă nu are 3 doze la dispoziție se mulțumește cu 2 doze, iar dacă nu are 2 doze la dispoziție se mulțumește cu una. Dacă Matei reciclează toate dozele goale care pot fi convertite în doze gratuite RedDrink pline cu băutură, după câte zile Matei nu mai poate să bea nicio doză de RedDrink?
`,
...
const problema_5: ProblemaType = {
    titlu: "Insule",
    enunt_markdown: `
Într-o matrice infinită (asemenea unui ocean nemărginit) în care pătratele albe reprezintă apă, iar pătratele negre reprezintă fâșii de pământ. Mai multe fâșii formează o insulă dacă se poate ajunge din orice fâșie din insulă în orice altă fâșie din insulă mergând fie pe orizontală, fie pe verticală pe fâșii de pământ adiacente (fără a merge pe diagonale). O insulă poate fi formată și dintr-o singură fâșie de pământ. De pe o fâșie dintr-o insulă nu putem ajunge pe o altă fâșie de pe altă insulă. Dimensiunea unei insule reprezintă numărul de fâșii care alcătuiesc insula. În figura de mai jos sunt 4 astfel de insule cu dimensiunile 1, 4, 4 și 24.
...
Câte unități de materie primă poate recicla maxim, dacă aruncă obiectele în tomberoane în mod optim? Materia primă reciclată este sticla aruncată la tomberonul de sticlă, plasticul aruncat la tomberonul de plastic și hârtia aruncată la tomberonul de hârtie.
...
const problema_7: ProblemaType = {
    titlu: "Fort Leon",
    enunt_markdown: `
Arhipelagul Fort Leon este format din 14 insule, majoritatea locuitorilor fiind programatori începători. Într-o zi cu soare, locuitorii arhipelagului s-au gândit că le-ar plăcea foarte mult să construiască mai multe poduri între insule astfel încât să poate pleca de pe orice insulă, să treacă o singură dată pe fiecare pod și să ajungă tot pe insula de unde au plecat. Pentru a-și îndeplini mai repede dorința, ei au scris un program care să-i ajute. Din păcate, ei au scris un program greșit, care le-a furnizat următoarea modalitate de construire a unor poduri între cele 14 insule:
`,
...
const problema_12: ProblemaType = {
    ...
    enunt_markdown: `
Considerăm șirurile de caractere de lungime 10 care conțin literele a si b, dar fără a conține trei apariții consecutive ale literelor a sau b. Spre exemplu, șirul 'ababababab' este un exemplu pozitiv (verifică regulile), iar șirul 'aaabababab' este un exemplu negativ (nu verifică regulile). Câte exemple pozitive de astfel de șiruri de caractere există?
`,
...
Considerând numerele a = A(123456789), b = B(123456789) și c = C(123456789), care dintre următoarele inegalități este adevărată?
`,
...
const problema_15: ProblemaType = {
    titlu: "Puncte șa",
    enunt_markdown: `
Într-o matrice M de dimensiune m×n se numește punct șa un element \`M[i][j]\` cu proprietatea că fie este minim pe linia i și maxim pe coloana j, fie este maxim pe linia i și minim pe coloana j. Dacă elementele matricei M sunt o permutare a mulțimii {1,2,...,m⋅n}, unde m ≥ 3 și n ≥ 3, atunci numărul maxim de puncte șa pe care poate să le aibă matricea M este:
`,
...
const problema_16: ProblemaType = {
    titlu: "Furnicuța",
    enunt_markdown: `
O furnicuță se deplasează pe o frunză, reprezentată printr-o matrice triunghiulară de dimensiune 2023, cu liniile și coloanele numerotate de la 1 (matricea are deci 2023 linii, iar fiecare linie i din matrice are i coloane). Ea ar vrea să calculeze în câte moduri poate ajunge din vârful frunzei, adică prima linie a matricei (din celula (1,1)), la baza frunzei, adică pe ultima linie a matricei (în oricare dintre celulele acesteia), știind că pentru că este așa de mică se poate deplasa dintr-o celulă a matricei doar un pas în jos sau un pas în dreapta-jos (din celula (i,j) în (i+1,j) sau (i+1, j+1)). Până să găsească ea o strategie de numărare, o omidă a venit și a mușcat dintr-o margine a frunzei celulele (x,1) unde x este multiplu de 100. Mai mult, omida pofticioasă a mușcat și celula (2001, 1999). Furnicuța nu mai știe acum să calculeze câte trasee poate face fără a trece prin celulele mușcate, așa ca va roagă pe voi să calculați care este cea mai mare putere a lui 2 care divide numărul acestor trasee.
`,
...
const problema_17: ProblemaType = {
    titlu: "Joc",
    enunt_markdown: `
X și Y joacă următorul joc: se pornește cu numărul p = 1, pe rând X și Y (începând cu X) multiplică numărul p cu un număr între 2 și 9 pe care îl aleg, câștigă primul care ajunge cu p >= n unde n este număr dat. Dacă și X și Y joacă optim cine câștigă jocul pentru un număr n dat? Dacă X și Y joacă 9 jocuri în care n este fixat pe rând la valorile: 37, 89, 167, 2920, 5832, 66666, 104970, 131313, 34012226 de câte ori câștigă X?
`,
...
const problema_20: ProblemaType = {
    ...
    enunt_markdown: `
Câte numere de forma 11...100...0 (cu m ≥ 1 ori 1 și n ≥ 1 ori 0) cu proprietatea că m+n≤15000 sunt multipli de 2023?
`,
