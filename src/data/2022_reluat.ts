...
`,
    ...
    raspuns: "cuprins între 61 și 90",
    dificultate: "usor"
};
...
const problema_5: ProblemaType = {
    titlu: "Permutare",
    enunt_markdown: `
Matei are următoarea permutare: $$9, 1, 2, 6, 5, 4, 3, 7, 10, 8$$
...
Orice element $A[i][j]$ al matricei este egal cu $((i+j) \\textbf{ mod } n) + 1$. Dacă $sumlin(i)$ reprezintă suma tuturor elementelor aflate pe linia $i$ și $sumcol(j)$ reprezintă suma tuturor elementelor aflate pe coloana $j$, atunci valoarea maximă a expresiei $|sumlin(i)-sumcol(j)|$, unde $i, j \\in \\{1,2, \\ldots,n\\}$, este egală cu:
`,
...
const problema_9: ProblemaType = {
    titlu: "Șir infinit",
    enunt_markdown: `
Considerăm șirul infinit $1234567891011121314151617181920\\dots$, obținut prin alipirea tuturor cifrelor tuturor numerelor naturale nenule. Considerând șirul respectiv ca fiind indexat de la $0$, atunci cifra aflată pe poziția $21052022$ este egală cu:
`,
    variante: ["1", "2", "3", "6", "9"],
    raspuns: "1",
    dificultate: "usor"
...
const problema_10: ProblemaType = {
    titlu: "Palindromuri",
    enunt_markdown: `
Uli a descoperit palindromurile și este foarte pasionată de ele, așa că dorește să calculeze câte palindromuri sunt mai mici ca $2022$. Totuși, după o experiență traumatizantă la ora de fizică, Uli nu mai suportă cifra $4$, așa că dorește să calculeze toate palindromurile între $0$ (inclusiv) și mai mici ca $2022$ care nu conțin cifra $4$. Câte astfel de palindromuri sunt?
`,
...
const problema_11: ProblemaType = {
    titlu: "Divizori",
    enunt_markdown: `
Câte numere între $1$ și $2.000.000.000$ au exact $5$ divizori?
`,
...
`,
    ...
    raspuns: "o monedă de $50$ de bani",
    dificultate: "mediu"
};
...
În mod inexplicabil, prețul acțiunilor la minutul $i$ este: $$(i^3 + 7 \\cdot i^2 + 3 \\cdot i + 84) \\textbf{ mod } 100$$

Având prețul acțiunilor pentru minutul $1$, $2$, $\\ldots$, $100$, Alex își pune următoarea întrebare: dacă ar putea, pe parcursul acestor $100$ de minute, să cumpere o acțiune și să o vândă mai târziu, care este profitul maxim pe care ar putea să îl obțină?
`,
...
const problema_15: ProblemaType = {
    ...
    enunt_markdown: `
Fie numerele "speciale": $$100, 356, 440, 1137, 1901$$

Definim puterea unui număr $x$ întreg ca fiind distanța față de cel mai apropiat număr special, respectiv valoarea minimă a modulului $|x - s|$, unde $s$ este unul dintre numerele speciale.
...
const problema_19: ProblemaType = {
    titlu: "Turnulețe",
    enunt_markdown: `
Ema are $22$ de cuburi astfel: $7$ verzi, $8$ albastre și $7$ roșii. Ea ar vrea să vadă câte turnulețe poate forma cu ele fără ca un cub verde să fie plasat imediat peste un cub roșu.
...
const problema_20: ProblemaType = {
    titlu: "Sosii",
    enunt_markdown: `
Doi oameni se numesc {\\emph{sosii}} dacă seamănă mult unul cu altul.
Evident, relația de {\\emph{a fi sosie}} este una de echivalență deoarece:
...
Numim o pereche $(A, B)$ {\\emph{minunată}} dacă îndeplinește simultan următoarele două condiții:

- $A < B$
- $A$ și $B$ sunt sosii.

Într-un grup de $N$ oameni Matei a observat că sunt exact $2022$ de perechi minunate.
